...
/// Human-readable language string for the status card.
/// e.g. "English", "Russian + English", "Auto-detect"
...
/// Human-readable model name for the status card.
/// e.g. "Base (English)", "Large Turbo"
...
/// Check if a model is english-only and thus incompatible with a given language.
pub fn is_model_english_only(model_idx: usize) -> bool {
    MODELS.get(model_idx).map(|m| m.english_only).unwrap_or(false)
}

/// Guard: prevent double-click launching two installs.
...
pub struct DictationConfig {
    pub primary_code: String,   // primary non-English code for dropdown selection
    pub also_english: bool,     // whether English is paired with another lang
...
/// Check if the voxtype config file exists on disk.
...
/// Check if voxtype is installed on the system.
...
/// Read the voxtype config file and extract settings.
pub fn read_config() -> Option<DictationConfig> {
    ...
    // Parse primary_code and also_english from the language string.
    // "en" -> primary_code="en", also_english=false
    // "auto" -> primary_code="auto", also_english=false
    // "ru" -> primary_code="ru", also_english=false
    // "en, ru" -> primary_code="ru", also_english=true
    ...
    debug_log!("[kb-center] dictation config: primary={}, also_en={}, model={}",
        primary_code, also_english, model);

    Some(DictationConfig { primary_code, also_english, model })
}

/// Check if the voxtype systemd user service is currently active.
...
/// Write voxtype config.toml with the given language and model.
/// Config is written BEFORE installation so voxtype reads it on first run.
...
pub fn write_config(lang_code: &str, model_id: &str, also_english: bool) -> bool {
    let dir = match home_dir() {
        ...
        None => {
            eprintln!("[kb-center] cannot write config: HOME not set");
            return false;
        }
    };
    if let Err(e) = std::fs::create_dir_all(&dir) {
        eprintln!("[kb-center] cannot create config dir: {}", e);
        return false;
    }
    ...
    match std::fs::write(&path, &config) {
        Ok(_) => {
            debug_log!("[kb-center] wrote config to {}", path);
            true
        }
        Err(e) => {
            eprintln!("[kb-center] failed to write config: {}", e);
            false
        }
    }
}
...
fn progress_file_path() -> String {
    ...
    format!("{}/kb-center-install-progress", run_dir)
}

/// Read install progress. Returns (percentage 0.0-1.0, status message).
...
/// Clear the progress file and reset the in-progress guard.
pub fn clear_progress() {
    let _ = std::fs::remove_file(progress_file_path());
    set_install_running(false);
}

/// Clear just the file (used before launching a new script).
fn clear_progress_file_only() {
    let _ = std::fs::remove_file(progress_file_path());
}

/// Clean up stale progress files from a previous crashed run.
/// Call once at startup.
pub fn cleanup_stale_progress() {
    let path = progress_file_path();
    if std::path::Path::new(&path).exists() {
        debug_log!("[kb-center] removing stale progress file");
        let _ = std::fs::remove_file(&path);
    }
    set_install_running(false);
}

/// Launch terminal to install packages + download model.
/// Config must already be written so voxtype reads the correct model.
/// Returns false if spawn failed or already running.
pub fn launch_install() -> bool {
    if INSTALL_IN_PROGRESS.swap(true, Ordering::SeqCst) {
        debug_log!("[kb-center] install already in progress, ignoring");
        return false;
    }
    debug_log!("[kb-center] launching install in terminal");
    ...
    let script = concat!(
        ...
        "    fi\n",
        "    # Check if it's a regular repo package first\n",
        "    if pacman -Si \"$pkg\" &>/dev/null; then\n",
        "        echo \"  Using pacman for $pkg...\"\n",
        "        sudo pacman -S --needed --noconfirm \"$pkg\" 2>&1 && return 0\n",
        "    fi\n",
        "    # Manual AUR build (makepkg is always available)\n",
        ...
        .spawn()
    {
        Ok(_) => true,
        Err(e) => {
            eprintln!("[kb-center] failed to spawn terminal: {}", e);
            set_install_running(false);
            // Write error to progress file so UI picks it up
            let path = progress_file_path();
...
/// Launch terminal to download a (possibly different) model.
/// Used when reconfiguring and the model changed.
/// Returns false if spawn failed or already running.
pub fn launch_model_download() -> bool {
    ...
    debug_log!("[kb-center] launching model download in terminal");
    clear_progress_file_only();
    let script = concat!(
        "PROG=\"${XDG_RUNTIME_DIR:-/tmp}/kb-center-install-progress\"\n",
        "cleanup() { echo \"0|Error: Interrupted\" > \"$PROG\"; exit 1; }\n",
...
/// Open the voxtype config file in the user's preferred editor.
...
pub fn open_config() {
    ...
    { "nvim" } else { "nano" };
    debug_log!("[kb-center] opening config with {}", editor);
    let _ = Command::new("terminal").args(["-e", editor, &cfg]).spawn();
}

/// Restart (or enable + start) the voxtype systemd user service.
pub fn restart_service() {
    debug_log!("[kb-center] restarting voxtype service");
