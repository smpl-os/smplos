...
pub fn load_theme_from_eww_scss(path: &str) -> ThemePalette {
    ...
    macro_rules! set_color {
        ($key:expr, $field:expr) => {
            if let Some(v) = vars.get($key).and_then(|s| parse_hex_color(s)) {
                $field = v;
            }
        };
    }

    set_color!("bg",          palette.bg);
    set_color!("fg",          palette.fg);
    set_color!("fg-dim",      palette.fg_dim);
    set_color!("accent",      palette.accent);
    set_color!("bg-light",    palette.bg_light);
    set_color!("bg-lighter",  palette.bg_lighter);
    ...
    if let Some(v) = vars.get("popup-opacity").and_then(|s| s.parse::<f32>().ok()) {
        palette.opacity = v;
    }
    if let Some(v) = vars.get("border-radius").and_then(|s| s.parse::<f32>().ok()) {
        palette.border_radius = v;
    }
