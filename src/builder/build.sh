...
setup_profile() {
    ...
    # ── Ventoy Normal-mode (EFI chainload) compatibility hook ────────────
    # When Ventoy boots our ISO in "Normal mode" it chainloads systemd-boot,
    # which passes archisosearchuuid=<uuid> to the kernel. The standard archiso
    # hook then scans block devices for /boot/<uuid>.uuid — but the ISO's FAT
    # partition is a *file* inside the Ventoy ExFAT volume, never exposed as a
    # block device. The scan always fails → emergency shell.
    #
    # This custom hook detects LABEL=Ventoy, loop-mounts the smplos ISO from it,
    # and sets archisodevice so archiso_mount_handler can find airootfs.sfs.
    # It only activates when LABEL=Ventoy is present (direct USB boot is unaffected).
...
#!/usr/bin/ash
# archiso_ventoy: Ventoy Normal-mode (EFI chainload) compatibility hook.
# Allows archisosearchuuid-based boot to work when booting via Ventoy.

run_hook() {
    # Skip if archiso_loop_mnt already has an img_dev set (GRUB2/loopback path)
    [ -n "$(getarg 'img_dev')" ] && return

    # Only needed when archisosearchuuid is present (systemd-boot path)
    local archisosearchuuid
    archisosearchuuid="$(getarg 'archisosearchuuid')"
    [ -z "$archisosearchuuid" ] && return

    # Quick Ventoy detection — blkid exits immediately, no 30-second timeout
    local ventoy_dev
    ventoy_dev="$(blkid -l -o device -t LABEL=Ventoy 2>/dev/null)"
    [ -z "$ventoy_dev" ] && return

    msg ":: Ventoy detected on ${ventoy_dev} — enabling ISO loop-mount fallback"
...
archiso_ventoy_mount_handler() {
    local newroot="${1}"
    local tmpdir="/run/archiso/vtoy"
    local iso_search iso_file loop_dev

    # vtoy_iso_name kernel param lets users override the search pattern
    iso_search="$(getarg 'vtoy_iso_name' 'smplos')"

    mkdir -p "${tmpdir}"
    modprobe exfat 2>/dev/null  # ExFAT support for Ventoy drive

    if ! mount -o ro,x-initrd.mount "${_archiso_ventoy_dev}" "${tmpdir}" 2>/dev/null; then
        echo "ERROR: Cannot mount Ventoy device ${_archiso_ventoy_dev}"
        launch_interactive_shell
        return
    fi

    # Find the ISO — search up to 3 levels deep for *smplos*.iso (or custom prefix)
    iso_file="$(find "${tmpdir}" -maxdepth 3 -name "*${iso_search}*.iso" 2>/dev/null | head -1)"
    if [ -z "${iso_file}" ]; then
        umount "${tmpdir}" 2>/dev/null
        echo "ERROR: No ISO matching '*${iso_search}*.iso' found on Ventoy drive"
        echo "       Add vtoy_iso_name=<prefix> to the kernel cmdline if your ISO has a different name"
        launch_interactive_shell
        return
    fi

    msg ":: Loop-mounting ${iso_file##*/}"
    if ! loop_dev="$(losetup --find --show --read-only -- "${iso_file}")"; then
        umount "${tmpdir}" 2>/dev/null
        echo "ERROR: Cannot loop-mount ${iso_file}"
        launch_interactive_shell
        return
    fi

    msg ":: ISO available at ${loop_dev}, handing off to archiso_mount_handler"
    export archisodevice="${loop_dev}"
    # Clear archisosearchfilename so archiso_mount_handler skips the device scan
    # and goes straight to mounting archisodevice (the ISO loop device).
...
VENTOYHOOK
    ...
    # Install hook: add_runscript includes the runtime hook in the initramfs image
...
help() {
    cat <<HELPEOF
  Enables Ventoy Normal-mode (EFI chainload) boot by loop-mounting the smplos
  ISO from the Ventoy ExFAT drive when archisosearchuuid device scan would fail.
HELPEOF
}
VENTOYINSTALL

    # Add archiso_ventoy to HOOKS right after archiso_loop_mnt
