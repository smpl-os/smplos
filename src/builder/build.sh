...
# Configures smplos-os-update (System Files section of the full OS update).
# Set SMPLOS_GITHUB_REPO to your GitHub owner/repo to enable OS file updates.
# The repo must publish releases with smplos-update-X.Y.Z.tar.gz assets.
SMPLOS_GITHUB_REPO="smplos/smplos"
RELCONF
...
build_update_tarball() {
    log_step "Building update tarball (smplos-update-${BUILD_VERSION}.tar.gz)"

    local update_name="smplos-update-${BUILD_VERSION}"
    local update_dir="$CACHE_DIR/update/$update_name"
    rm -rf "$update_dir"
    mkdir -p "$update_dir"
    ...
    if [[ -d "$SRC_DIR/shared/bin" ]]; then
        ...
        cp "$SRC_DIR/shared/bin/"* "$update_dir/bin/" 2>/dev/null || true
        chmod +x "$update_dir/bin/"* 2>/dev/null || true
    fi

    # Shared configs → config/
    mkdir -p "$update_dir/config"
    [[ -d "$SRC_DIR/shared/configs" ]] && \
        cp -r "$SRC_DIR/shared/configs/"* "$update_dir/config/" 2>/dev/null || true

    # Compositor hypr configs → config/hypr/
    local comp_dir="$SRC_DIR/compositors/$COMPOSITOR"
    if [[ -d "$comp_dir/hypr" ]]; then
        mkdir -p "$update_dir/config/hypr"
        cp -r "$comp_dir/hypr/"* "$update_dir/config/hypr/" 2>/dev/null || true
    fi
    [[ -d "$comp_dir/configs" ]] && \
        cp -r "$comp_dir/configs/"* "$update_dir/config/" 2>/dev/null || true

    # bindings.conf → config/hypr/ AND config/smplos/ (single source of truth)
    if [[ -f "$SRC_DIR/shared/configs/smplos/bindings.conf" ]]; then
        mkdir -p "$update_dir/config/hypr" "$update_dir/config/smplos"
        cp "$SRC_DIR/shared/configs/smplos/bindings.conf" "$update_dir/config/hypr/bindings.conf"
        cp "$SRC_DIR/shared/configs/smplos/bindings.conf" "$update_dir/config/smplos/bindings.conf"
    fi

    # EWW widgets → config/eww/
    if [[ -d "$SRC_DIR/shared/eww" ]]; then
        mkdir -p "$update_dir/config/eww"
        cp -r "$SRC_DIR/shared/eww/"* "$update_dir/config/eww/" 2>/dev/null || true
    fi

    # Themes → themes/ (skip _templates dirs)
    if [[ -d "$SRC_DIR/shared/themes" ]]; then
        mkdir -p "$update_dir/themes"
        for theme_dir in "$SRC_DIR/shared/themes"/*/; do
            [[ "$(basename "$theme_dir")" == _* ]] && continue
            cp -r "$theme_dir" "$update_dir/themes/"
        done
    fi

    # VERSION
    echo "$BUILD_VERSION" > "$update_dir/VERSION"

    # ── Generate manifest.txt ────────────────────────────────────────────────
    # Files generated at runtime (theme-set outputs, auto-generated bindings).
    # These are NEVER overwritten by an OS update — they get regenerated by
    # theme-set and generate-messenger-bindings in the post-deploy hook.
    local skip_files=(
        "config/hypr/theme.conf"
        "config/hypr/hyprlock-theme.conf"
        "config/hypr/messenger-bindings.conf"
        "config/eww/theme-colors.scss"
        "config/btop/themes/current.theme"
        "config/fish/theme.fish"
        "config/dunst/dunstrc.active"
        "config/rofi/smplos-launcher.rasi"
        "config/foot/theme.ini"
    )

    {
        echo "# smplOS update manifest v$BUILD_VERSION"
        echo "# Format: <policy> <path>"
        echo "# always = overwrite unconditionally (users never edit these)"
        echo "# smart  = only overwrite if user hasn't modified the file"
        echo "# skip   = never touch (generated at runtime by theme-set etc.)"
        echo ""

        # Scripts: always update
        find "$update_dir/bin" -maxdepth 1 -type f 2>/dev/null | sort | while read -r f; do
            echo "always  bin/$(basename "$f")"
        done

        # Theme templates: always update (generated baked outputs are skip)
        find "$update_dir/themes" -type f 2>/dev/null | sort | while read -r f; do
            echo "always  themes/${f#$update_dir/themes/}"
        done

        # Config files: smart by default, skip for generated outputs
        find "$update_dir/config" -type f 2>/dev/null | sort | while read -r f; do
            local rel="config/${f#$update_dir/config/}"
            local policy="smart"
            for skip in "${skip_files[@]}"; do
                [[ "$rel" == "$skip" ]] && { policy="skip"; break; }
            done
            echo "$policy  $rel"
        done

    } > "$update_dir/manifest.txt"

    local manifest_lines
    manifest_lines=$(grep -c '^[^#]' "$update_dir/manifest.txt" || true)
    log_info "Manifest: $manifest_lines file entries"

    # ── Pack tarball ─────────────────────────────────────────────────────────
    mkdir -p "$RELEASE_DIR"
    local tarball="$RELEASE_DIR/${update_name}.tar.gz"
    tar -czf "$tarball" -C "$CACHE_DIR/update" "$update_name/"
    log_info "Update tarball: ${update_name}.tar.gz ($(du -sh "$tarball" | cut -f1))"
    log_info "Upload this file to the GitHub release alongside the ISO."
}
